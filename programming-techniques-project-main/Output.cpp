...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	...
	int y = vcell * UI.CellHeight + UI.ToolBarHeight;
	///TODO: implement the following function as described in Output.h file
...
void Output::CreateDesignModeToolBar() const  //OD
{
	UI.InterfaceMode = MODE_DESIGN;
	...
	//added
	MenuItemImages[ITM_DELETE_OBJECT] = "images\\Delete_Icon.jpg  ";
	MenuItemImages[ITM_CUT_CARD] = "images\\Cut_Icon.jpg ";
	MenuItemImages[ITM_COPY_CARD] = "images\\Copy_Icon.jpg ";
	MenuItemImages[ITM_SAVE_GRID] = "images\\Save_Game.jpg ";
...
void Output::CreatePlayModeToolBar() const //OD
{
	...
	//added
	MenuItemImages[ITM_START_GAME] = "images\\Menu_Start.jpg ";
	MenuItemImages[ITM_EXIT_PLAY_MODE] = "images\\Menu_Exit.jpg ";
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Input_Icon.jpg ";
...
void Output::PrintPlayersInfo(string info) //OOZ
{
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX, cellStartY + UI.CellHeight, cellStartX + UI.CellWidth, cellStartY);
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int ladderLength = abs(fromCell.VCell() - toCell.VCell());

	for (int i = 1; i <= ladderLength; ++i)
	{
		pWind->DrawLine(x12, toStartY + i * UI.CellHeight, x34, toStartY + i * UI.CellHeight);
	}
