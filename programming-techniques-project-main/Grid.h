...
public:
	...
	void SaveAll(ofstream &fOut, GameObjectType type);
	void LoadAll(ifstream& fIn);
	Cell* GetCellFromPosition(CellPosition position);
	bool IsSnakeStartSameASLadderEnd(CellPosition Cellx); 
	bool AreGameObjectsOverlapping(GameObject* Obj) const; 
	Player** GetPlayerist();
	void ClearGrid();
